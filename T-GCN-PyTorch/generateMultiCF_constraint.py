...
node_index = 1138 #882/ 881 / 1034 /1800/2185/1138
# Set target outcome
...
road_index_df= pd.read_csv("Data/ventura_road/st3_link_index.csv", header = None)
...
def get_k_nearest(individual, k=1):
...
def objective_function(cf_poi, cf_lanes, poi_data, lanes_data,speedlimit_data, orig_data, target_speed_increase, individual):
    ...
    poi_array = torch.from_numpy(poi_data[0,0]).float()- orig_data[:,:,:,14][:,:,road_index][0,0]
    poi_diff = torch.sum(poi_array[poi_array>0])+torch.sum(poi_array[poi_array<=0]*(-1.0))
    lane_array = torch.from_numpy(lanes_data[0,0]).float() - orig_data[:,:,:,15][:,:,road_index][0,0]
    lanes_diff = torch.sum(lane_array[lane_array<0]*(-1.0))+torch.sum(lane_array[lane_array>0])
    ...
    speedlimit_diff = torch.abs(torch.from_numpy(speedlimit_data[0,0]).float() - orig_data[:,:,:,16][:,:,road_index][0,0]).mean()
...
pareto_front_df.to_csv('Data/moc/pareto_front_t52.csv', index=False)
pd.DataFrame(pareto_front[0]).to_csv('Data/moc/individual_t52.csv', index=False)
